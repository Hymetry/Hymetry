...
class ProductAreaColorsTests(SimpleTestCase):
    def test_explicit_color_accepts_hex_and_rejects_unsafe_css(self):
        self.assertEqual(explicit_product_area_color({'color': '#AbC123'}), '#AbC123')
        self.assertEqual(
            explicit_product_area_color({'color': '\"><svg/onload=alert(1)>'}),
            '',
        )
